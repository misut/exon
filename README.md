...
| `exon add [--dev] --cmake <name> --repo <url> --tag <tag> --targets <targets> [--install-targets <targets>] [--install-package <name>] [--option K=V] [--install-option K=V] [--shallow false]` | Add a raw CMake dependency |
...
`[dependencies.cmake.<name>.install]` describes how the dependency is consumed
...
The same global policy can be set with `EXON_CMAKE_INSTALL_CACHE=prefer`.
